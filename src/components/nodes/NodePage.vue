    ...
    <main>
      <header class="main-header">
        <div class="main-title">
          <h1>{{ node.name }}</h1>
          <h2>
            <a :href="nodeHost" target="_blank" rel="noopener noreferrer">
              {{ node.host }}
            </a>
          </h2>
        </div>
        ...
        <div>
          <div class="flex">
              ...
              <ul>
                <li>
                  ...
                  <router-link
                    ...
                    :to="{
                      name: 'platform',
                      params: { platform: node.thefederation_platform.id },
                    }"
                    :title="platformTitle"
                  >
                    <strong>
                ...
                <li>
                  Version: <strong>{{ node.version }}</strong>
                </li>
                <li>
                  Open signups:
                  <strong>{{ node.open_signups ? "Yes" : "No" }}</strong>
                </li>
                <!-- <li>Services: <strong>{{ node.services }}</strong></li> -->
                <!-- <li>Protocols: <strong>{{ node.protocols }}</strong></li> -->
                <li>
                  Country: <strong>{{ node.country }}</strong>
                </li>
                <li>
            ...
            <div class="col2">
              <ul>
                <li>
                  Users: <strong><Number :number="stats.users_total" /></strong>
                </li>
                <li>
                  Last 6 months active users:
                  <strong><Number :number="stats.users_half_year" /></strong>
                </li>
                <li>
                  Last month active users:
                  <strong><Number :number="stats.users_monthly" /></strong>
                </li>
                <li>
                  Posts: <strong><Number :number="stats.local_posts" /></strong>
                </li>
                <li>
                  Comments:
                  <strong><Number :number="stats.local_comments" /></strong>
                </li>
              </ul>
            </div>
          </div>
      ...
      <Charts v-if="node" :item="node.host" :node-id="node.id" type="node" />
    </main>
...
export default {
  ...
  computed: {
    diasporaStatisticsUrl() {
      if (
        this.node.thefederation_platform.name === "diaspora" &&
        this.node.host
      ) {
    ...
    platformTitle() {
      return this.node.thefederation_platform.display_name
        ? this.node.thefederation_platform.display_name
        : this.node.thefederation_platform.name;
    },
    services() {
      const services = [];
      for (const o of Object.assign(
        [],
        this.node.thefederation_node_services,
      )) {
