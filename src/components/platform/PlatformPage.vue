        ...
        <div>
          <div class="flex">
            <div class="col2">
              ...
              <div class="flex">
                <div v-if="platform.website" class="col2 center">
                  <a
                ...
                <div v-if="platform.code" class="col2 center">
                  <a
                    :href="platform.code"
                    class="btn"
                    target="_blank"
            ...
            <div class="col2">
              <ul>
                ...
                <li>
                  Users: <strong>{{ globalStats.users_total || "" }}</strong>
                </li>
                <li>
                  Last 6 months users:
                  <strong>{{ globalStats.users_half_year || "" }}</strong>
                </li>
                <li>
                  Last month users:
                  <strong>{{ globalStats.users_monthly || "" }}</strong>
                </li>
                <li>
                  Posts: <strong>{{ globalStats.local_posts || "" }}</strong>
                </li>
                <li>
                  Comments:
                  <strong>{{ globalStats.local_comments || "" }}</strong>
                </li>
              </ul>
            </div>
          </div>
...
const query = gql`
  query PlatformDetails(
    ...
    nodeStats: thefederation_stat(
      ...
      order_by: { users_monthly: desc_nulls_last }
      limit: $pageSize
      offset: $pageOffset
    ) {
      users_total
    ...
    nodeCount: thefederation_stat_aggregate(
      where: {
        date: { _eq: $yesterday }
        thefederation_node: {
          platform_id: { _eq: $id }
    ...
    filteredCount: thefederation_stat_aggregate(
      where: {
        date: { _eq: $yesterday }
        thefederation_node: {
          platform_id: { _eq: $id }
    ...
    thefederation_stat_aggregate(
      where: {
        thefederation_platform: { id: { _eq: $id } }
        date: { _eq: $yesterday }
      }
    ) {
      aggregate {
...
export default {
  apollo: {
    platforms: {
      ...
      result({ data }) {
        ...
        this.nodes = data.nodeStats.map(
          ({ thefederation_node: node, ...avg }) => ({
            ...node,
        ...
        this.globalStats =
          data.thefederation_stat_aggregate.aggregate.avg || {};
      },
      variables() {
        const date = new Date();
  ...
  computed: {
    title() {
      return this.platform.display_name
        ? this.platform.display_name
        : this.platform.name || "";
    },
    loadMoreEnabled() {
