    ...
    <main>
        ...
        <div>
          <div class="flex">
            <div class="col2">
              ...
              <div class="flex">
                <div v-if="protocol.website" class="col2 center">
                  <a
            ...
            <div class="col2">
              <ul>
                <li>
                  Nodes: <strong><Number :number="nodes.length" /></strong>
                </li>
                <li>
                  Users:
                  <strong><Number :number="globalStats.users_total" /></strong>
                </li>
                <li>
                  Last 6 months users:
                  <strong
                    ><Number :number="globalStats.users_half_year"
                  /></strong>
                </li>
                <li>
                  Last month users:
                  <strong
                    ><Number :number="globalStats.users_monthly"
                  /></strong>
                </li>
                <li>
                  Posts:
                  <strong><Number :number="globalStats.local_posts" /></strong>
                </li>
                <li>
                  Comments:
                  <strong
      ...
      <section class="tile">
        ...
        <div class="overflow-x">
          <NodesTable :nodes="filteredTableNodes" />
          <ApolloLoader :loading="$apollo.loading" />
        </div>
      </section>
    </main>
...
export default {
  apollo: {
    queries: {
      ...
      result({ data }) {
        ...
        this.nodes =
          data.thefederation_protocol_by_pk.thefederation_node_protocols;
        this.tableNodes =
          data.thefederation_protocol_by_pk.thefederation_node_protocols.map(
            (n) => n.thefederation_node,
          );
        this.globalStats =
          data.thefederation_protocol_by_pk.thefederation_stats_aggregate
            .aggregate.avg || {};
      },
      variables() {
        const date = new Date();
  ...
  computed: {
    ...
    filteredTableNodes() {
      ...
      return this.tableNodes.filter((node) =>
        (node.name || "").toLowerCase().includes(term),
      );
    },
    title() {
      return this.protocol.display_name
        ? this.protocol.display_name
        : this.protocol.name || "";
    },
  },
  methods: {
